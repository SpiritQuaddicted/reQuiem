...
void Z_Print (memzone_t *zone)
{
	...
	for (block = zone->blocklist.next ; ; block = block->next)
	{
		...
		if ((byte *)block + block->size != (byte *)block->next)
			Con_Print ("ERROR: block size does not touch the next block\n");
		if (block->next->prev != block)
			Con_Print ("ERROR: next block doesn't have proper back link\n");
		if (!block->tag && !block->next->tag)
			Con_Print ("ERROR: two consecutive free blocks\n");
	}
}
...
typedef struct
{
	...
	char	name[8];		// JDH: many Hunk_Alloc's have >8-letter names, which means they 
							//		aren't getting a null terminator! (but it seems to work...)
} hunk_t;
...
void Hunk_Print_f (cmd_source_t src)
{
	Hunk_Print (false);
}
...
void Q_strncpy_nowarn (char *dest, const char *src, int count)
{
	while (*src && count--)
		*dest++ = *src++;
...
#define HUNK_CHECKSPACE(size)	\
	if (hunk_size - hunk_low_used - hunk_high_used < (size))	\
	{	\
	  	Sys_Error (va("Not enough RAM allocated.  Try using \"-mem %d\" on the command line.",	\
					(host_parms.memsize >> 20)*2));			/* JDH: recommend double current amount	*/	\
	}

#define HUNK_INITBLOCK(h, size)	\
	memset (h, 0, size);		/* JDH: is this really necessary? */	\
	h->size = size;					\
	h->sentinal = HUNK_SENTINAL;	\
	Q_strncpy_nowarn (h->name, name, sizeof(h->name));		/* JDH: Q_strncpy often triggers my overflow check */
...
//	if (size < 0)		// JDH: size is unsigned now
//		Sys_Error ("Hunk_Alloc: bad size: %i", size);
		
	size = sizeof(hunk_t) + ((size + 15) & ~15);

	HUNK_CHECKSPACE(size);
...
// we are clearing up space at the bottom, so only allocate it late
	...
	if ((newc = Cache_TryAlloc(c->size, true)))
	{
//		Con_Print ("cache_move ok\n");
...
============
*/
void Cache_Flush (cmd_source_t src)
{
...
*/
#ifdef _DEBUG
void Cache_Print (cmd_source_t src)
{
	cache_system_t	*cd;
	size_t total = 0;
	...
	{
		Con_Printf ("  %08X to %08X: %8i bytes : %s\n", cd, (byte *)cd + cd->size, cd->size, cd->name);
		total += cd->size;
	}

	Con_Printf ("Total cache size: %d bytes\n", total);
}
#endif
...
void Cache_Init (void)
{
	...
	Cmd_AddCommand ("flush", Cache_Flush, 0);
#ifdef _DEBUG
	Cmd_AddCommand ("cache", Cache_Print, 0);
#endif
}
...
#ifdef _DEBUG
	Cmd_AddCommand ("hunk_print", Hunk_Print_f, 0);
#endif
}
