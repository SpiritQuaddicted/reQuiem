...
#ifdef HEXEN2_SUPPORT
	if (hexen2)
		friction *= sv_player->v.friction;
#endif

	if (trace.fraction == 1.0)
	{
		friction *= sv_edgefriction.value;
	}
...
==============
*/
void SV_WaterFlightMove (qboolean isflight)
{
	int	i;
	vec3_t	wishvel;
...
// user intentions
	...
	for (i=0 ; i<3 ; i++)
	{
		...
		if ( isflight ) wishvel[i] += up[i] * cmd.upmove;
	}
	...
	if (!isflight)
	{
		...
		if (hexen2)
		{
			...
			val = (int) sv_player->v.playerclass;
			if (val == CLASS_DEMON)   // Paladin Special Ability #1 - unrestricted movement in water
				wishspeed *= 0.5;
			else if (val != CLASS_PALADIN)   // Paladin Special Ability #1 - unrestricted movement in water
				wishspeed *= 0.7;
			else
			{
				val = (int) sv_player->v.level;
				switch (val)
				{
					case 1:
						wishspeed *= 0.75;
...
void SV_ClientThink (void)
{
	...
	{
#ifndef RQM_SV_ONLY
// joe: removed DropPunchAngle() which is moved to view.c instead similarly to QW.
// copying the already counted values from cl.punchangle gets it work during demoplayback as well.
		VectorCopy (cl.punchangle, sv_player->v.punchangle);
#endif
	}
...
// walk
...
#ifdef HEXEN2_SUPPORT
	if (hexen2 && (sv_player->v.movetype == MOVETYPE_FLY))
	{
		SV_FlightMove ();
		return;
	}
	...
	if (sv_player->v.movetype == MOVETYPE_NOCLIP && sv_altnoclip.value)		// JDH: from Fitz
		SV_NoclipMove ();
	else
		SV_AirMove ();
}
...
nextmsg:
		...
		while (1)
		{
			...
			case clc_stringcmd:
				s = MSG_ReadString ();
			#ifdef HEXEN2_SUPPORT
				/*if (hexen2 && host_client->privileged)
				{
					ret = 2;
					Cbuf_InsertText (s, SRC_CLIENT);
				}
				else*/
			#endif
				...
				#ifdef HEXEN2_SUPPORT
					|| (hexen2 && !Q_strncasecmp(s, "playerclass", 11))
				#endif
					)
				{
					ret = 1;
