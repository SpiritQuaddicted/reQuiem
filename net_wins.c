...
{ 
	...
	ret = (BOOL)PeekMessageA (&msg, NULL, 0, 0, PM_REMOVE);
...
// so we can run on Win 3.1, where there isn't necessarily Winsock)
	...
	if ((i = COM_CheckParm("-ip")))
	{
		if (i < com_argc-1)
		{
			...
			Q_strcpy (my_tcpip_address, com_argv[i+1], sizeof(my_tcpip_address));
		}
		else
		{
	...
	{
		myAddr = INADDR_ANY;
		Q_strcpy (my_tcpip_address, "INADDR_ANY", sizeof(my_tcpip_address));
	}

	if ((net_controlsocket = WINS_OpenSocket(0)) == -1)
	{
//joe		Con_Print ("WINS_Init: Unable to open control socket\n");
		if (--winsock_initialized == 0)
			pWSACleanup ();
		return -1;
	}
...
//joe	Con_Print ("Winsock TCP/IP Initialized\n");
	tcpipAvailable = true;
...
static int PartialIPAddress (const char *in, struct qsockaddr *hostaddr)
{
	...
	buff[0] = '.';
	b = buff;
	Q_strcpy (buff+1, in, sizeof(buff)-1);
	if (buff[1] == '.')
		b++;
...
int WINS_Connect (int socket, const struct qsockaddr *addr)
{
	return 0;
}
...
int WINS_Read (int socket, byte *buf, int len, const struct qsockaddr *addr)
{
...
int WINS_Broadcast (int socket, const byte *buf, int len)
{
	int	ret;
	...
	if (socket != net_broadcastsocket)
	{
		...
		if (ret == -1)
		{
			Con_Print ("Unable to make socket broadcast capable\n");
			return ret;
		}
	}
...
int WINS_Write (int socket, const byte *buf, int len, const struct qsockaddr *addr)
{
	int	ret;
...
const char *WINS_AddrToString (const struct qsockaddr *addr)
{
...
int WINS_GetNameFromAddr (const struct qsockaddr *addr, char *name, int bufsize)
{
	...
	if (hostentry)
	{
		Q_strcpy (name, (char *)hostentry->h_name, bufsize/*NET_NAMELEN - 1*/);
		return 0;
	}

	Q_strcpy (name, WINS_AddrToString (addr), bufsize);
...
int WINS_GetSocketPort (const struct qsockaddr *addr)
{
