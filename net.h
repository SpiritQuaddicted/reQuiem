...
typedef struct
{
	...
	int 		(*Connect) (int socket, const struct qsockaddr *addr);
	int 		(*CheckNewConnections) (void);
	int 		(*Read) (int socket, byte *buf, int len, const struct qsockaddr *addr);
	int 		(*Write) (int socket, const byte *buf, int len, const struct qsockaddr *addr);
	...
	int 		(*GetNameFromAddr) (const struct qsockaddr *addr, char *name, int bufsize);
	...
	int			(*GetSocketPort) (const struct qsockaddr *addr);
...
#if !defined(_WIN32 ) && !defined (__linux__) && !defined(MACOSX)
#ifndef htonl
