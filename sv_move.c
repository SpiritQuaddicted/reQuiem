...
=================
*/
void SV_set_move_trace(trace_t *trace)
{
...
*pr_global_ptrs.trace_normal is set to the normal of the blocking wall
=============
*/
#ifdef HEXEN2_SUPPORT
...
#ifdef HEXEN2_SUPPORT
	if ((!hexen2) || (!((int)ent->v.flags & FL_NOZ) && !((int)ent->v.flags & FL_HUNTFACE)))
#endif
	{
	// try one move with vertical motion, then one without
		for (i=0 ; i<2 ; i++)
		{
		...
		#endif
			{
				...
				#ifdef HEXEN2_SUPPORT
					if ((hexen2) && ((int)ent->v.flags & FL_HUNTFACE))
						dz += PROG_TO_EDICT(ent->v.enemy)->v.view_ofs[2];	// Go for face
				#endif		
...
#ifdef HEXEN2_SUPPORT
	set_trace_plane = ((hexen2) && ((int)ent->v.flags & FL_SET_TRACE) != 0);
...
//Reset trace_plane_normal
	if (hexen2)
		VectorCopy(vec3_origin, *pr_global_ptrs.trace_plane_normal);
#endif
...
// bump around...
#ifdef HEXEN2_SUPPORT
	// should be effectively the same as Quake version
	if ( hexen2 )
	{
